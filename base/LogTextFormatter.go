...
// LogTextFormatter formats logs into text
...
func (f *LogTextFormatter) init(entry *logrus.Entry) {
	if entry.Logger != nil {
...
func (f *LogTextFormatter) Format(entry *logrus.Entry) ([]byte, error) {
	var b *bytes.Buffer
...
func (f *LogTextFormatter) printColored(b *bytes.Buffer, entry *logrus.Entry, keys []string, timestampFormat string) {
	var levelColor int
	switch entry.Level {
...
func (f *LogTextFormatter) needsQuoting(text string) bool {
...
func (f *LogTextFormatter) appendKeyValue(b *bytes.Buffer, key string, value interface{}) {
	if b.Len() > 0 {
		b.WriteByte(' ')
	}
	b.WriteString(key)
...
func (f *LogTextFormatter) appendValue(b *bytes.Buffer, value interface{}) {
